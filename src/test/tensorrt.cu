...
void test(const TestConfig &testConfig, cv::Mat & image, ICudaEngine *engine, IExecutionContext *context);
...
void test(const TestConfig &testConfig, cv::Mat & image, ICudaEngine *engine, 
	IExecutionContext *context, std::vector<PowerReadingDevice> &pwr_devices,
	std::string csv_filename)
{
  int inputBindingIndex, outputBindingIndex;
  inputBindingIndex = engine->getBindingIndex(testConfig.inputNodeName.c_str());
  ...
  ostringstream conv;
  conv << avgTime;
  to_csv(csv_filename, pwr_devices, testConfig.GetNetName()+"-avgTime: "+conv.str());
